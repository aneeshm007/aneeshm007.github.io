...
function [x,y,p,q]=brusselator(xl,xr,yb,yt,tb,te,M,N,tsteps,Dp,Dq,C,K)
Dp=Dp;
Dq=Dq;
C=C;
K=K;

fp=@(x,y) 1 + cos(pi*x)*cos(pi*y);
fq=@(x,y) 2 + cos(2*pi*x)*cos(2*pi*y);
...
vidobj=VideoWriter('k=7.8', 'MPEG-4');
vidobj.FrameRate=30;
open(vidobj)
for tstep=1:tsteps
  ...
  for it=1:3
    ...
    for i=2:m-1
      for j=2:n-1
        ...
        DF1(mn+i+(j-1)*m,mn+i+j*m)=-Dq/colok^2;
